...
// Mostly feature gaps or output/order differences to chip away at; the
// panic-subquery group is a known bug (the engine should never panic).
var gsiExpectedNonPass = map[string]string{
	...
	"case_gsi_subqexp.json[5]":              "panic-subquery",
	...
	"case_gsi_withs.json[8]":                "panic-subquery",
	"case_gsi_withs.json[9]":                "panic-subquery",
...
var gsiGroupWhy = map[string]string{
	...
	"subquery":          "correlated / nested / derived-table subquery gaps: an aggregate inside a correlated subquery (SUM(...) over an outer field -- 'nil item'); a correlated subquery whose FROM is a subquery+WITH; a correlated subquery in the projection (SELECT (SELECT ... WHERE = outer) ...); a subquery USE KEYS (SELECT RAW ...); and a derived-table FROM (SELECT ...) UNNEST ... under UNION. (Plain correlated SELECT / EXISTS / IN subqueries do work.)",
	...
	"panic-subquery":    "subqexp[5], withs[8,9]: a subquery nested directly inside another subquery's RAW projection (SELECT RAW (SELECT RAW a)) or a WITH binding inside such a projection panics at exec (nil deref). KNOWN BUG -- the engine should never panic; conversion succeeds, so the fix is in subquery execution, not glue conversion",
}
