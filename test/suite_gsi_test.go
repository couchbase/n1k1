...
var gsiGroupWhy = map[string]string{
	...
	"window-results-differ":   "window[22]: the query runs but the output differs from cbq -- LAG/LEAD with a default value (`LAG(c5,3,'None')` yields the default, not NULL, when the offset is out of range), NTH_VALUE ... FROM LAST, and STDDEV/VARIANCE over an explicit frame; none fully honored yet",
