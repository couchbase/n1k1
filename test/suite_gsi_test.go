...
var gsiExpectedNonPass = map[string]string{
	...
	// cbq's own results for the FULL window surface. 14 of the 31 cases now pass: the 5
	// non-OVER aggregate cases (0,1,2,3,5), 4 multi-column RANGE cases (7,8,24,25),
	// window[4] (ORDER BY a non-projected aggregate), and 4 window-over-a-grouped-
	// aggregate cases (9,10,26,27 -- SUM(COUNT(x)) OVER (PARTITION BY MIN(y) ORDER BY
	// MAX(z))). The 17 OVER cases below are grouped by proximate blocker. The core window
	// machinery (ROWS/RANGE/GROUPS aggregates + ranking + offset + NULLS ordering +
