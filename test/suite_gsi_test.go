...
var gsiExpectedNonPass = map[string]string{
	...
	// cbq's own results for the FULL window surface. 25 of the 31 cases now pass: the 19
	// core cases -- the 5 non-OVER aggregate cases (0,1,2,3,5), 4 multi-column RANGE cases
	// (7,8,24,25), window[4] (ORDER BY a non-projected aggregate), 4
	// window-over-a-grouped-aggregate cases (9,10,26,27), window[6,23] (SUM over a
	// non-numeric column -> NULL), window[12,20] (RATIO_TO_REPORT + DISTINCT-in-window),
	// and window[29] (named-window reference that adds a frame, resolved now that
	// REWRITE_PHASE1 runs before the semantics check) -- plus window[11,13,19,21,28,30],
	// frame-position cases (FIRST_VALUE / NTH_VALUE) whose tie-break within a group of
	// ORDER-BY-key peers matches cbq's stored row order. Those six were once tracked
	// non-pass out of caution (the peer tie-break is implementation-defined, so a pass
	// wasn't guaranteed across lanes/builds); they've passed reliably, so they're no
	// longer listed. The core window machinery (ROWS/RANGE/GROUPS aggregates + ranking +
	// offset + NULLS ordering + multi-column peer frames + ORDER-BY/PARTITION-BY/OVER an
	// aggregate + empty frames + SUM/AVG NULL-on-no-numeric + RATIO_TO_REPORT +
	// DISTINCT-in-window + named-window reference frames) works -- see glue/window_test.go
