...
var gsiExpectedNonPass = map[string]string{
	...
	// cbq's own results for the FULL window surface. 10 of the 31 cases now pass: the 5
	// non-OVER aggregate cases (0,1,2,3,5), 4 multi-column RANGE cases (7,8,24,25), and
	// window[4] (ORDER BY a non-projected aggregate). The 21 OVER cases below are grouped
	// by proximate blocker. The core window machinery (ROWS/RANGE/GROUPS aggregates +
	// ranking + offset + NULLS ordering + multi-column peer frames + ORDER-BY an
	// aggregate) works -- see glue/window_test.go + glue/order_nulls_test.go; these gaps
	// are unsupported window funcs / frames, window-over-a-grouped-aggregate, output
	// differences, or nondeterministic tie order. None panic (conv rejects an
	// un-computable window function gracefully).
	"case_gsi_window.json[13]": "window-nondeterministic",

	"case_gsi_window.json[9]":  "window-unsupported",
	"case_gsi_window.json[10]": "window-unsupported",
	...
	"case_gsi_window.json[26]": "window-unsupported",
	"case_gsi_window.json[27]": "window-unsupported",
	"case_gsi_window.json[30]": "window-unsupported",

	"case_gsi_window.json[6]":  "window-results-differ",
	...
	"case_gsi_window.json[23]": "window-results-differ",
...
var gsiGroupWhy = map[string]string{
	...
	"window-unsupported":      "window[9,10,12,14,15,16,17,18,20,26,27,30]: a window function/frame n1k1 doesn't compute natively, so conv rejects the statement (NA) rather than emit a boxed cbq window Evaluate (which panics on n1k1's plain rows). Covers a window OVER a grouped aggregate (`SUM(COUNT(c)) OVER (... ORDER BY MAX(c))`, 9/10/26/27), RATIO_TO_REPORT, COUNTN, NTH_VALUE ... FROM LAST, a numeric-offset RANGE with multi-column ORDER BY, and DISTINCT-in-window over a non-catalog aggregate. Each bundles several window funcs across w1..w7, so one unsupported func gates the whole SELECT",
	"window-results-differ":   "window[6,11,19,21,22,23]: the query runs (all its window funcs are wired, incl. ORDER BY a window function now) but the output differs from cbq's recorded results -- SUM over a non-numeric column, GROUPS-frame peer semantics, EXCLUDE GROUP/TIES, STDDEV/VARIANCE over an explicit frame, or DISTINCT-in-window, none of which n1k1 fully honors yet",
