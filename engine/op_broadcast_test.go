...
func TestOpBroadcastEquivalence(t *testing.T) {
	...
	// Run the broadcast once; group the interleaved tagged labelResults by tag. Slot 0
	...
	for _, row := range collectRows(t, broadcast, broadcastVars()) {
		if len(row) != 2 {
			t.Fatalf("broadcast labelResult has %d slots, want 2: %v", len(row), row)
		}
	...
	if len(got["all"]) != n {
		t.Fatalf("tag all: got %d labelResults, want %d", len(got["all"]), n)
	}
	if len(got["none"]) != 0 {
		t.Fatalf("tag none: got %d labelResults, want 0", len(got["none"]))
	}
	if len(got["some"]) != n-13 { // a in 13..19
		t.Fatalf("tag some: got %d labelResults, want %d", len(got["some"]), n-13)
	}
	if len(got["missing"]) != 0 {
		t.Fatalf("tag missing: got %d labelResults, want 0 (MISSING is non-truthy)", len(got["missing"]))
	}
}
