...
| `.index [list\|show <name>\|rebuild [<name>]\|suggest [<ks>]\|help]` | Secondary-index family (`.indexes` = `.index list`): list indexes from `.n1k1/catalog.json` with stats, show one, force-rebuild (all/one), suggest candidate indexes from a doc sample (emits an editable catalog fragment), or help. Owned by DESIGN-indexing.md "CLI control". |
