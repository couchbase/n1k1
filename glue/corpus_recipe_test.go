...
`
	...
	if len(r.Fixture.Expect) != 2 || r.Fixture.Expect[0].Tag != "ET-12345" {
...
// detector whose Tag is the filename stem and whose Stmt is the whole body -- the
...
func TestParseRecipePlain(t *testing.T) {
	...
	if r.Tag != "errors" {
		t.Errorf("Tag = %q, want errors (filename stem)", r.Tag)
	}
	if r.HasFixture || r.HasExpect {
...
func TestLoadCorpus(t *testing.T) {
	...
	if recipes[0].Tag != "ET-1" || !recipes[0].HasFixture {
		t.Errorf("recipe[0] = %+v, want Tag ET-1 with a fixture", recipes[0])
	}
	if recipes[1].Tag != "b_plain" || recipes[1].HasFixture {
		t.Errorf("recipe[1] = %+v, want Tag b_plain with no fixture", recipes[1])
	}
...
func TestRewriteExpectRoundTrip(t *testing.T) {
	...
	if !r.HasExpect || len(r.Fixture.Expect) != 1 || r.Fixture.Expect[0].Tag != "ET-9" {
	...
	out2 := RewriteExpect(out, []Finding{{Tag: "ET-9", Evidence: json.RawMessage(`{"sev":"ERROR","n":2}`)}})
	if !strings.HasPrefix(out2, head) {
		t.Fatalf("second RewriteExpect changed the head:\n%q", out2)
	}
	r2, _ := ParseRecipe("x.sql++", out2)
	if canonicalJSON(r2.Fixture.Expect[0].Evidence) != `{"n":2,"sev":"ERROR"}` {
		t.Fatalf("in-place replace failed: %s", r2.Fixture.Expect[0].Evidence)
	}
}

// TestDiffFindings: equal sets diff clean regardless of order and evidence key order;
// a missing and an unexpected finding are reported on their respective sides.
func TestDiffFindings(t *testing.T) {
	expected := []Finding{
		{Tag: "T", Evidence: json.RawMessage(`{"a":1,"b":2}`)},
		{Tag: "T", Evidence: json.RawMessage(`{"a":3}`)},
	}
	// Same set, reordered + different key order in evidence -> no diff.
	actual := []Finding{
		{Tag: "T", Evidence: json.RawMessage(`{"a":3}`)},
		{Tag: "T", Evidence: json.RawMessage(`{"b":2,"a":1}`)},
	}
	...
	actual2 := []Finding{
		{Tag: "T", Evidence: json.RawMessage(`{"a":1,"b":2}`)},
		{Tag: "T", Evidence: json.RawMessage(`{"a":99}`)},
	}
	m, u := DiffFindings(expected, actual2)
	if len(m) != 1 || canonicalJSON(m[0].Evidence) != `{"a":3}` {
		t.Fatalf("missing = %v, want the {a:3} finding", m)
	}
	if len(u) != 1 || canonicalJSON(u[0].Evidence) != `{"a":99}` {
...
`)
	...
	for _, f := range findings {
		if f.Tag != "ET-1" {
			t.Errorf("finding tag = %q, want ET-1", f.Tag)
		}
	}
