...
func wrapPrepare(labels base.Labels, lines []string) string {
	...
	// glue is referenced by the body only for a boxed expression (glue.ExprStr) or a
	// datastore island (glue.DatastoreOp). A fully-native query whose datastore
	// leaves go through the Pipe (emit.PipeMode) references no glue: the file links
	// engine+base only and needs no n1ql build tag (cbq-off) -- the standalone,
	// cbq-free target (DESIGN-prepare.md phase 2).
	usesGlue := strings.Contains(emitted, "glue.")

	var b strings.Builder
	if usesGlue {
		b.WriteString("//go:build n1ql\n\n") // the island/boxed path links cbq (n1ql-gated glue)
	}
	b.WriteString("// Code generated by glue.Prepare (n1k1 .prepare). DO NOT EDIT.\n")
	// Stamp the n1k1 source provenance (git SHA + any uncommitted edits) the
	// generated code targets, so a compiled artifact is traceable + reproducible.
	fmt.Fprintf(&b, "// %s\n", ProvenanceCapture(ProvenanceSourceDir()).Stamp())
	b.WriteString("//\n")
	...
	b.WriteString("// lzYieldErr. The caller supplies lzVars; datastore leaves dispatch through\n")
	if usesGlue {
		b.WriteString("// glue.DatastoreOp islands baked inline.\n")
	} else {
		b.WriteString("// lzVars.Ctx.Pipe (a base.DatastorePipe) -- so this file links engine+base only.\n")
	}
