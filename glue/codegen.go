...
// (the .codegen dot-command / Session.Codegen), reusing the same emitter the
// compiler differential tests use to build test/tmp. See
// DESIGN-extensions-codegen.md.
//
// It is emit-only + gated + with an interpreter fallback: Codegen decides, via
// Codegenable, whether a converted base.Op tree can be lowered to standalone Go,
...
//
// OUT OF SCOPE (documented future work in DESIGN-extensions-codegen.md): the pipe
...
// Codegenable reports whether a converted base.Op tree can be lowered to
...
// gate-check first and only emit -- via Codegen -- when ok.
func Codegenable(op *base.Op) (ok bool, reason string) {
	if op == nil {
...
// lets the exprStr codegen path (emit.OpToLines' ExprCatalog["exprStr"]) emit a
// glue.ExprStr(...) call that re-parses at the generated program's runtime.
// Returns false if any exprTree param can't be serialized (so Codegen can bail).
//
...
// CodegenFuncName is the name of the entry-point function in the emitted .go
// source. It matches DESIGN-extensions-codegen.md's Run(vars, yield, yieldErr).
const CodegenFuncName = "Run"

// wrapCodegen assembles emit.OpToLines' body into a complete, parseable Go file:
...
// projection labels travel as a companion CodegenLabels var so a caller can
...
func wrapCodegen(labels base.Labels, lines []string) string {
	emitted := strings.Join(lines, "")

	var b strings.Builder
	b.WriteString("//go:build n1ql\n\n")
	b.WriteString("// Code generated by glue.Codegen (n1k1 .codegen). DO NOT EDIT.\n")
	b.WriteString("//\n")
	b.WriteString("// Run executes the fused query plan, yielding each result row's base.Vals\n")
	b.WriteString("// to lzYieldVals (columns ordered per CodegenLabels) and any error to\n")
	...
	fmt.Fprintf(&b, "// CodegenLabels are the projection labels for the rows Run yields.\n")
	fmt.Fprintf(&b, "var CodegenLabels = %#v\n\n", labels)
...
// Codegen runs parse -> plan -> convert for a statement and, when the converted
// op tree is Codegenable, emits standalone Go source for it (emit-only). On
...
//
// This is the user-facing surface behind the CLI's .codegen / -codegen (see
...
func (s *Session) Codegen(stmt string) (goSource string, ok bool, reason string, err error) {
	...
	// Gate on the UN-stringified tree: Codegenable inspects ["exprTree", <expr>]
	// params to decide native vs boxed. (stringifyExprTrees below would erase that
	// distinction by turning every expr into text.)
	if ok, reason = Codegenable(conv.TopOp); !ok {
		return "", false, reason, nil
	}

	// Rewrite live exprTree objects to exprStr text so the tree is expressible as
	// generated source. Codegenable already proved every expr is native, so an
	// unserializable exprTree here is unexpected -- treat it as not-codegenable.
	if !stringifyExprTrees(conv.TopOp) {
		return "", false, "expression not serializable to text", nil
	}

	return wrapCodegen(conv.TopOp.Labels, emit.OpToLines(conv.TopOp)), true, "", nil
}
