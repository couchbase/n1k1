...
// TestCorpusGateStandalone: a standalone (window) entry gated on a NECESSARY
...
func TestMultiQueryGateStandalone(t *testing.T) {
	// grep -C1 style window flag: standalone (has OVER), labelResults only near an ERROR.
	...
	run := func(t *testing.T, logs string, d MultiQueryEntry) *CompiledMultiQueryEntries {
		sess := gateSession(t, logs)
		cc, err := sess.MultiQueryCompile([]MultiQueryEntry{d})
		if err != nil {
			t.Fatalf("MultiQueryCompile: %v", err)
		}
		if len(cc.Standalone) != 1 {
			t.Fatalf("expected the window entry to be standalone; got %+v", cc.Standalone)
		}
		return cc
	}

	// (1) gate present in the keyspace -> not skipped, entry runs and finds context.
	...
	if len(f) != 2 { // both rows are within 1 line of the ERROR at code=1
		t.Errorf("gate present: expected 2 context labelResults, got %d (%+v)", len(f), f)
	}

	// (2) gate ABSENT from the keyspace -> the entry is skipped (its sort/window is
	// never run), producing zero labelResults, and the skip is recorded (not silent).
	...
	if len(f) != 0 {
		t.Errorf("gate absent: expected 0 labelResults, got %+v", f)
	}
