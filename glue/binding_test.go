...
func labelResultTags(fs []LabelResult) map[string]int {
	m := map[string]int{}
	for _, f := range fs {
		m[f.Label]++
	}
...
// run against TWO differently-named bundles, produces the expected labelResults from
...
func TestBindingTwoBundles(t *testing.T) {
	...
	for _, bundle := range []struct {
		...
		t.Run("bundle_"+bundle.name, func(t *testing.T) {
			...
			// Expected labelResults, identical for both bundles (equivalent contents):
			//   idx_error   -> 2 ERROR rows (i1, i3)
			//   idx_timeout -> 1 row (i3)
			//   big_order   -> 2 rows (o1 total 150, o3 total 300)
			got := labelResultTags(labelResults)
			want := map[string]int{"idx_error": 2, "idx_timeout": 1, "big_order": 2}
			for label, n := range want {
				if got[label] != n {
					t.Errorf("bundle %s: label %q labelResults = %d, want %d (all=%v)",
						bundle.name, label, got[label], n, got)
				}
			}
			for label := range got {
				if _, ok := want[label]; !ok {
					t.Errorf("bundle %s: unexpected labelResult label %q", bundle.name, label)
				}
			}
			// t.Logf("bundle %s labelResults: %v", bundle.name, got)
		})
	}
}
