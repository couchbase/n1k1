...
const jsModuleHoist = "\n;(function(){var fs=(exports&&exports.functions)||[];" +
	...
	"if(k==='aggregate'||k==='agg'){globalThis[n+'_init']=f.init;globalThis[n+'_update']=f.update;globalThis[n+'_final']=f.final;if(f.merge){globalThis[n+'_merge']=f.merge;}if(f.snapshot){globalThis[n+'_snapshot']=f.snapshot;}}" +
...
func registerJSModule(bundle, source, sourcePath string) (names []string, err error) {
	...
	for _, e := range entries {
		...
		if kind == "aggregate" {
			...
			// single-file *.agg.js path (ext_jsvm_agg.go). Optional `snapshot` →
			// a cheaper mid-flight live view (Agg.ResultLive).
