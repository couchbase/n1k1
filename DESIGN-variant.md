...
manifest of entries, each self-describing its `name`, `marshal`, and (later) `kind` — e.g.
...
This puts the `marshal` axis per-export in the manifest — so filenames carry no
