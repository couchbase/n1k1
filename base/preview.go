...
// PreviewMaxGroups bounds how many groups a single checkpoint snapshots (first-N),
...
var PreviewMaxGroups = 64

// previewableAggs is the set of aggregate handler names whose partial value is
...
// scalar Result verbatim, so they are previewable too.
var previewableAggs = map[string]bool{
...
func (r *PreviewRow) begin(op string) {
	r.Op = op
	r.keyBuf = r.keyBuf[:0]
	r.Key = r.Key[:0]
	r.aggBuf = r.aggBuf[:0]
...
func (r *PreviewRow) SetKey(keyBytes []byte) {
...
func (r *PreviewRow) AggBufTail() []byte { return r.aggBuf[len(r.aggBuf):] }
...
// Preview is the per-request, reused buffer for the latest live-aggregate
// snapshot. It owns a growable pool of PreviewRow (rows), each with its own
// reused byte buffers; a snapshot resets the live count to zero and re-fills via
// Next(), so steady-state re-decoding allocates nothing. Held on Stats.Preview.
type Preview struct {
	rows []PreviewRow // backing pool, reused across checkpoints
	n    int          // number of live rows filled this snapshot
}

// Next returns the next reusable PreviewRow for a source to fill (its buffers
// reset but their capacity retained). Grows the pool only until it reaches the
// steady-state row count.
func (p *Preview) Next(op string) *PreviewRow {
	if p.n == len(p.rows) {
...
func (p *Preview) FinishRow(r *PreviewRow) { r.finish() }

// Full reports whether the snapshot has reached PreviewMaxGroups live rows, so a
// source can stop walking the group map early.
func (p *Preview) Full() bool { return p.n >= PreviewMaxGroups }

func (p *Preview) reset() { p.n = 0 }

// Rows returns the live rows filled by the last RefreshPreview. Valid until the
// next refresh; a retaining consumer must copy.
func (p *Preview) Rows() []PreviewRow {
	if p == nil {
		return nil
	}
	return p.rows[:p.n]
...
// GroupPreviewSnapshot copies out a bounded, coherent sample of an OpGroup's live
...
// previewable aggregate handler names (from engine.StatsGroupAggNames); vars is
// passed through to Agg.Result (the previewable aggs ignore it). Allocation-free
...
func GroupPreviewSnapshot(dst *Preview, op string, set *store.RHStore, aggNames []string, vars *Vars) {
