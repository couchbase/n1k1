...
func (c *cli) helpExtensions() {
	...
	c.hline("")
	c.hline("TABLE SOURCE  (foo.stream.js) — emit rows, used in FROM. File  ints.stream.js :")
	...
	c.hline("carry an `examples` array of {in, out}: it self-documents the function AND golden-")
	c.hline("tests it, so a function can't silently rot.")
	c.hline("  n1k1 -ext ./my-udfs -c 'SELECT initials(\"Grace Hopper\")'   # load + run")
	c.hline("  .extensions list             # all loaded, every kind (incl. built-in macros)")
	c.hline("  .extensions show <name>      # print one extension's full source (@name for a macro)")
	c.hline("  .extensions examples [name]  # print the examples")
	c.hline("  .extensions test [name]      # run them + check every in -> out")
