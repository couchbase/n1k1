...
func (c *cli) helpVectors() {
	...
	c.hline("                          batch => 64, opts => {\"dim\":16}) AS r;")
	c.hline("")
	...
	c.hline("  - @vectorize_field rows come back as {id, text, vec} -- the embedded text is")
	c.hline("    ALWAYS named `text`, never the source field's name. `SELECT r.line` yields")
	c.hline("    MISSING and SQL++ silently drops the column; check the materialized target")
	c.hline("    with  SELECT * FROM <target> LIMIT 1.")
	c.hline("  - Keep batches modest (the default is 64): huge pages can crash the model")
	c.hline("    server (a local ollama reset connections at 256 x ~1.5KB prompts).")
	c.hline("  - ollama: use /api/embed (batch API, UNIT-normalized vectors), NOT the older")
	c.hline("    /api/embeddings (raw magnitudes) -- un-normalized vectors quietly skew")
	c.hline("    anything assuming euclidean ~ cosine (e.g. k-means: examples/kmeans/).")
...
func (c *cli) helpExtensions() {
	...
	c.hline("  With \"json\" an object/array argument is a fully-plain JS value at every depth")
	c.hline("  (nested fields, arrays, and typeof all behave) — navigate x.a.b.c directly.")
	c.hline("  PERF/FUSION: a UDF in the SELECT list boxes only that column (per-row cbq cost); the scan")
	c.hline("  still fuses + index-prunes. Move UDFs to a downstream .multi compose node if that cost bites.")
	...
	c.hline("  the built-in EJSON_DECODE / EJSON_DECIMAL / EJSON_UNWRAP.")
	c.hline("  The builtin_decimal (DECIMAL_*, incl. the DECIMAL_SUM aggregate) and builtin_ejson")
	c.hline("  (EJSON_*) modules ship IN the binary — auto-loaded, no -ext needed. (They're named")
	c.hline("  builtin_*.js so an embedder can glob them too; see glue.RegisterExtensionGlob.)")
	c.hline("")
	...
	c.hline("")
	c.hline("GOLDEN EXAMPLES — every kind (scalar, aggregate, source, extract plugin, macro) can")
...
func (c *cli) helpModes() {
	...
	c.hline("  list        each row's values joined by | separator")
	c.hline("")
...
func (c *cli) helpKeyspaces() {
	...
	c.hline("• `FROM `./data/**/*.json`` — an inline doublestar glob keyspace. A bare")
	c.hline("  record-file path works too: `FROM `/tmp/x/window.jsonl`` (one-file keyspace).")
	c.hline("")
	c.hline("Multiple sources at once — give several paths (dirs, files, or globs) and each")
	c.hline("becomes its own keyspace, joinable in one query:")
	c.hline("  n1k1 ~/Drive/**/*.json ~/Documents 'reports=~/q/reports/**'")
	c.hline("A bare source's name is the deepest literal path segment (case preserved): a file's")
	c.hline("stem, a dir's basename, or a glob's base (the part before the first * ? [). So")
	c.hline("~/Drive/reports/** + ~/Drive/ecommerce -> `reports` + `ecommerce` (not two `Drive`s).")
	c.hline("Two sources yielding the same name is an error -- name one with name=path. A bare dir")
	c.hline("recurses (its files are unioned); .open <src>... does the same in the REPL. Sources may")
	c.hline("mix kinds -- a local dir, an Iceberg table, an s3:// Parquet -- federated in one query.")
	c.hline("A -sources <file> (JSON/YAML/TOML) is the richer form: for a path with spaces, or")
	c.hline("per-source options -- formats (a per-source -formats lockdown) and namespace (then query")
	c.hline("it as FROM <namespace>:<keyspace>). .open @<file> loads one in the REPL.")
	c.hline("  sources: { drive: \"~/Google Drive/**\", orders: { path: ~/o, namespace: analytics } }")
	c.hline("")
	c.hline("Piped JSON — with -c or -f, PIPED stdin becomes a keyspace named `stdin`")
	c.hline("when there's NO datastore, or a `-` (or name=-) source names it. (It needs -c/-f, so")
	c.hline("it never clashes with `echo 'SELECT ...' | n1k1`, which runs stdin as STATEMENTS.)")
	c.hline("stdin is NDJSON (one JSON doc per line); it's buffered, so multi-scan queries work:")
	c.hline("  cat data.jsonl | n1k1 -c 'SELECT s.type, COUNT(*) FROM stdin s GROUP BY s.type'")
	c.hline("  # crack open a nested JSON field readably (the JSON-in-JSON fix):")
	c.hline("  n1k1 -c '.multi show --queries builtin:census.sql++' \\")
	c.hline("    | n1k1 -mode yaml -c 'SELECT RAW s.sql FROM stdin s'")
	c.hline("  # join a file corpus against piped data ('-' = stdin, name it with name=-):")
	c.hline("  cat metrics.jsonl | n1k1 -c 'SELECT h.host, m.cpu FROM hosts h JOIN metrics m ON h.id=m.id' ./hosts metrics=-")
	c.hline("")
	c.hline("Object stores (S3 / GCS / Azure) — point the datastore arg at a remote URI. The")
	c.hline("keyspace is named after the table dir or file stem; credentials come from the usual")
	c.hline("environment (AWS_*, GOOGLE_APPLICATION_CREDENTIALS, AZURE_*). Only the footer +")
	c.hline("needed column chunks transfer (ranged reads), so it never downloads whole files.")
	c.hline("  # Iceberg table (bare dir: current metadata auto-resolved by listing):")
	c.hline("  n1k1 -c 'SELECT COUNT(*) FROM orders' s3://bucket/warehouse/db/orders")
	c.hline("  # ...or an explicit metadata JSON (works on any backend):")
	c.hline("  n1k1 -c 'SELECT * FROM orders LIMIT 5' \\")
	c.hline("       gs://bucket/db/orders/metadata/00007-<uuid>.metadata.json")
	c.hline("  # A single Parquet object (s3://, gs://, abfs://):")
	c.hline("  n1k1 -c 'SELECT AVG(t.price) FROM t' abfs://fs@acct.dfs.core.windows.net/data/t.parquet")
	c.hline("  Iceberg time-travel works too: FROM `orders@<snapshot-id-or-timestamp>`.")
	c.hline("")
	c.hline("Test-drive on real PUBLIC data, no credentials (Ookla Open Data speedtest tiles,")
	c.hline("~227MB Parquet in us-west-2). AWS_NO_SIGN_REQUEST skips signing; AWS_REGION must")
	c.hline("match the bucket (AWS 301-redirects otherwise). COUNT reads only the footer, so it")
	c.hline("returns in ~1s without downloading the file -- copy/paste:")
	c.hline("  AWS_NO_SIGN_REQUEST=1 AWS_REGION=us-west-2 n1k1 -c \\")
	c.hline("    'SELECT COUNT(*) AS n FROM `2019-01-01_performance_fixed_tiles`' \\")
	c.hline("    s3://ookla-open-data/parquet/performance/type=fixed/year=2019/quarter=1/2019-01-01_performance_fixed_tiles.parquet")
	c.hline("  -> {\"n\":4877036}")
	c.hline("")
	c.hline(".tables / .keyspaces lists them with a framing tag (jsonl / plugin=<name> /")
...
func (c *cli) helpMeta() {
	...
	c.hline("")
	c.hline("So a labelResult is externally chase-able: dd/tail -c+<byte_offset>, sed -n <line_start>p,")
...
func (c *cli) helpTempKeyspaces() {
	...
	c.hline("big files once into small labelResult keyspaces, then correlate them, all in one")
