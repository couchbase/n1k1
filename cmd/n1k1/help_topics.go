...
func (c *cli) helpKeyspaces() {
	...
	c.hline("  # Public buckets (no credentials): AWS_NO_SIGN_REQUEST=1, and set AWS_REGION to")
	c.hline("  # the bucket's region (AWS 301-redirects otherwise):")
	c.hline("  AWS_NO_SIGN_REQUEST=1 AWS_REGION=us-west-2 \\")
	c.hline("    n1k1 -c 'SELECT COUNT(*) FROM tiles' s3://ookla-open-data/parquet/.../tiles.parquet")
