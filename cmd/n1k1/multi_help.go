...
COMMANDS
  ...
  .multi lint --queries <dir> [--bind <manifest>] [--census]  authoring report card (compiles, does NOT run); --census escalates to data-aware lint: cross-check each query's referenced fields against a top-level field inventory of its data (computed by plain SQL++, independent of any census fork) — flags a field the data lacks (birth-in-error) + data fields no query reads
  ...
  .multi run --queries "builtin:census.sql++?keyspace=<ks>[&type-field=f&time-field=f&depth=1|2&exclude=a,b&first-id=1]" [--bind <m>]   time-aware key-space census, as pure forkable SQL++ (see/fork it with .multi show): per (type, path, val_type) -> docs, first/last-seen (schema drift). Mergeable core only: coverage = read-time divide vs builtin:census_totals.sql++; first-id=1 opts into per-record provenance; cursor it + fold windows for an ongoing census. The bundled census_agg() JS aggregate is the same census as ONE mergeable aggregate (SELECT census_agg(r) FROM <ks> r) -- forkable for example values / histograms / deeper walks (fork extensions/functions/builtin_census_agg.js). Both are CI-checked cell-for-cell against a frozen test-only oracle (the retired native census)
  builtin SQL++ queries ship EMBEDDED in the binary (versioned, parameterized *.sql++ files): each declares its own artifact version (pin with builtin:<name>@<version>; a wrong pin errors, never silently runs a different query) and its params with defaults (?k=v&...; an unknown param errors and names the declared set). .multi show --queries builtin:<name> prints the exact rendered SQL -- what shows is what runs, and what you fork
...
FLAGS
  ...
  --sql              .multi explain only: render the queries as pretty SQL++ instead of the op tree.
                     ...
                     shared wake-gate. A fuse-eligible query alone on its keyspace, standalone queries,
                     and rejected queries get their own sections (so it's honest when nothing fuses).
                     Each query carries a provenance comment (keyspace / lane / index literal) + inline
                     '-- hint:' advice; a macro query shows its FINAL expanded SQL++, bracketed by
                     '-- BEGIN/END expansion of @macro'.
