...
COMMANDS
  ...
  .multi run --queries "builtin:census?keyspace=<ks>[&type-field=f&time-field=f&depth=1|2&exclude=a,b]" [--bind <m>]   time-aware key-space census (a built-in queries source): per (type, path, val_type) -> docs, coverage, first/last-seen (schema drift); cursor it for an ongoing census. builtin:census.sql++ is the same census core as pure, forkable SQL++ (mergeable core only: coverage = read-time divide vs builtin:census_totals.sql++; first-id=1 opts into per-record provenance; oracle-checked -- see it with .multi show)
