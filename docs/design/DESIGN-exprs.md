...
| `base64_decode_string` | `engine/expr_str.go` + `base/str.go` | base64-decode → force a STRING (never re-parse as JSON, unlike the boxed `BASE64_DECODE`); two lifted buffers (decode scratch + encode output) avoid aliasing. **Native-only**: `glue/base64_decode_string.go` is just the cbq name registration (the parser resolves function names against cbq's registry — even JS UDFs must register), and its `Evaluate` *delegates to the same* `base.StrBase64DecodeInto`, so there is no second implementation. DESIGN-data.md §9 |
