...
- [~] Object-store backend (S3/GCS/Azure): Iceberg-over-S3 reads land, incl. CLI wiring —
      `n1k1 s3://…/table/metadata/NNNNN.metadata.json` is a FROM-able keyspace (env-sourced
      creds; `.tables`/`SELECT`/time-travel). Remaining: full mock-S3 read test (dep-blocked),
      current-metadata auto-resolution from a bare `s3://…/table` dir (needs listing),
...
endpoint. **CLI wiring also landed:** `n1k1 s3://…/table/metadata/NNNNN.metadata.json`
makes that remote table a FROM-able keyspace (`.tables`, `SELECT`, time-travel). Still
deferred: a full mock-S3 read of real object bytes (blocked on vendoring a mock — see
Testing below), auto-resolving the *current* metadata from a bare `s3://…/table` dir URI
(needs object listing), and standalone remote-Parquet (a `FROM s3://…/x.parquet`).
...
- **Current-metadata auto-resolution over object stores: ⬜ NEXT** — accept a bare
  `s3://bucket/table` dir URI (no explicit `metadata.json`) by LISTING `…/metadata/` to pick
  `version-hint.text` / the greatest `*.metadata.json` (the object-store analog of the local
  `records.IcebergTableMetadata`). iceberg-go's blob IO exposes no listing, so this needs
  `gocloud.dev/blob.List` (already a dep) — best landed with the mock-S3 test below.
